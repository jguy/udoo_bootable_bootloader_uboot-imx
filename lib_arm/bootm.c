...
#endif
#ifdef CONFIG_BOARD_REVISION_TAG
	setup_board_revision_tag (bd);
#endif
#ifdef CONFIG_SETUP_MEMORY_TAGS
...
#ifdef CONFIG_BOARD_REVISION_TAG
void setup_board_revision_tag(bd_t *bd) {
	params->hdr.tag = ATAG_BOARDREV;
	params->hdr.size = tag_size(tag_boardrev);
	memcpy (params->u.boardrev.revision, bd->board_revision, 4);
	params = tag_next (params);
}
#endif /* CONFI_BOAR_REVISION_TAG */
